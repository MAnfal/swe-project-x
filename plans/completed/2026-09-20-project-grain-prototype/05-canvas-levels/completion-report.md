...
| A change card can be expanded and collapsed from the keyboard alone | yes | The card is a single `<button>`, so Enter and Space fire it natively — that comes from the element, not from any ARIA attribute; `←`/`Esc` collapse. Exercised with the keyboard only: `ArrowRight` on the focused node → Level 2, `ArrowLeft` → back to Level 2 from Level 3, `ArrowLeft` again → Level 1, each confirmed by reading the level indicator back out of the DOM. |
