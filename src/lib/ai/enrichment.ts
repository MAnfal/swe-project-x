...
import {
  ...
  MAX_LABEL_CHARS,
} from './enrichment-record.ts';

/**
 * Re-exported so chunk 03's public surface is unchanged. The definitions moved to
 * `enrichment-record.ts` because `src/lib/view/derive.ts` — which client components
 * import — needs `enrichmentKey` and `isFallbackEnrichment`, and this module imports
 * `ai` (Principle 2).
 */
export {
  enrichmentKey,
